...
class UpdateForm(FlaskForm):
    id = HiddenField('id')
    name = StringField('雜目錄名稱', [
        DataRequired(),Length(max=50)],
        render_kw={'class':'form-control','placeholder':'雜目錄名稱50字內,用以辨識,須唯一'})
    parent = SelectField('上層雜目錄',
        choices = [('','-- 請選擇 --')],
        render_kw={'class':'form-control'})    
    #is_leaf = BooleanField('最底層雜目錄', 
    #    render_kw={'class':'form-check-input'})
    is_leaf = RadioField('最底層雜目錄', 
        choices = [('1', '是'),('0', '否')],
    ...
    def validate_is_leaf( form, field):
        ...
        if form.id.data and field.data is False:
            with app.db_session.session_scope() as session: 
                has_articles = session.query(SubProductCategory).filter(SubProductCategory.parent_id==form.id.data).count()
                if has_articles>0:
