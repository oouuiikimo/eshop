...
class RepoSubProductCategory(BaseRepo):
    def __init__(self):
        super().__init__()
        self.title = "商品雜目錄維護"
    ...
    def get_search_filters(self,search):
        ...
        if search:
            if 'name' in search and search['name']: 
                filters.append(SubProductCategory.name.like(f'%{search["name"]}%') )   
            if 'is_leaf' in search and search['is_leaf']: 
                filters.append(SubProductCategory.is_leaf==search['is_leaf'])    
            if 'parent' in search and search['parent']: 
                #raise Exception(str(_get_all_child_id(int(search['parent']))))
                filters.append(SubProductCategory.id.in_(self._get_all_child_id(int(search['parent']))))   
    ...
    def find(self, id=None):
        ...
        if id:
            with app.db_session.session_scope() as session:
                db_item = session.query(SubProductCategory).get(id)
                form_item = self.form_mapper(db_item)
        else:
            db_item = SubProductCategory()
            form_item = self.form_mapper(db_item)
        return form_item
    ...
    def update(self,item,id=None):
        ...
        try:
            with app.db_session.session_scope() as session:
                ...
                if item.parent:
                    parent = session.query(SubProductCategory).filter(SubProductCategory.id==int(item.parent)).first()
    ...
    def get_tree(self,id=None):
        ...
        with app.db_session.session_scope() as session:
            if id:
                ...
                return [(str(i.id),self.get_cat_path(i)) for i in session.query(SubProductCategory).filter(SubProductCategory.id.notin_(child),SubProductCategory.is_leaf==False).all()]
            else:
                #is_leaf 不能出現
